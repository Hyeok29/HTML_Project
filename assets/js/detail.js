...
$(function () {
    ...
    var ec2cpu_avg_El = document.querySelector('#ec2cpu_avg'),
        ec2cpu_avg_Config = {
            series:
                [{
                    name: '7일전(<small>' + date_week + ')<br/>동시간 평균값',
                    data:
                    ...
                    {
                        name: '전일(<small>' + date_yesterday + ')<br/>동시간 평균값',
                        data:
                            $.map(ec2cpu_data, function (k) {
    ...
    var ec2cpu_max_El = document.querySelector('#ec2cpu_max'),
        ec2cpu_max_Config = {
            series:
                [{
                    name: '7일전 <small>(' + date_week + ')<br/>동시간 최대값',
                    data:
                        $.map(ec2cpu_data, function (k) {
                            return k['week_max'];
                        })
                    ...
                    {
                        name: '전일 <small>(' + date_week + ')<br/>동시간 최대값',
                        data:
                            $.map(ec2cpu_data, function (k) {
    ...
    var ecsmem_use_El = document.querySelector('#ecsmem_use'),
        ecsmem_use_Config = {
            series:
                [{
                    name: '7일전(<small>' + date_week + ')<br/>동시간 사용율',
                    data:
                    ...
                    {
                        name: '전일(<small>' + date_yesterday + ')<br/>동시간 사용율',
                        data:
                            $.map(ecsmem_data, function (k) {
                                return k['yesterday_value'];
                            })
                    },
                    {
                        name: '현재 사용율',
    ...
    if (typeof ecsmem_use_El !== undefined && ecsmem_use_El !== null) {
        var chart = new ApexCharts(ecsmem_use_El, ecsmem_use_Config);
        chart.render();
    }
    // ecs_mem Ends
    ...
    var rdsfssChartEl = document.querySelector('#rdsfss_line'),
        rdsfssChartConfig = {
            chart: {
                height: 400,
                type: 'area',
            ...
            series: [
                {
                    name: rdsfss_data.resource,
                    data: [rdsfss_data.week_avg, rdsfss_data.yesterday_avg, rdsfss_data.current_avg]
                }
            ],
            xaxis: {
                categories: [
                    "7일전 (" + date_week + ") 동시간 평균",
                    "전일 (" + date_yesterday + ") 동시간 평균",
                    "현재 평균"
                ]
            },
            fill: {
            ...
            yaxis: {
                labels: {
                    formatter: function (value) {
                        var result = addCommas(formatBytes(value));
                        return result;
                    }
                },
                opposite: isRtl
    ...
    if (typeof rdsfssChartEl !== undefined && rdsfssChartEl !== null) {
        var rdsfssChart = new ApexCharts(rdsfssChartEl, rdsfssChartConfig);
        rdsfssChart.render();
    }
    // rds_fss Ends
    ...
    var rdsdc_bar_El = document.querySelector('#rdsdc_bar'),
        rdsdc_bar_Config = {
            series:
                [{
                    name: '7일전(<small>' + date_week + ')<br/>동시간 연결 수',
                    data:
                    ...
                    {
                        name: '전일(<small>' + date_yesterday + ')<br/>동시간 연결 수',
                        data:
                            $.map(rdsdc_data, function (k) {
                                return k['yesterday_value'];
                            })
